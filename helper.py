...
def decrypt_file(keys_string, input_path, output_path):
    """Decrypts the given input file using mp4decrypt and saves it to the output path."""
    cmd = f'mp4decrypt {keys_string} --show-progress "{input_path}" "{output_path}"'
    os.system(cmd)
    if os.path.exists(output_path):
        return output_path
    else:
        raise FileNotFoundError("Decryption failed: file not found.")

def merge_video_audio(video_path, audio_path, output_path):
    """Merges video and audio files using ffmpeg and saves the result to the output path."""
    cmd = f'ffmpeg -i "{video_path}" -i "{audio_path}" -c copy "{output_path}"'
    os.system(cmd)
    if os.path.exists(output_path):
        return output_path
    else:
        raise FileNotFoundError("Merging failed: file not found.")
